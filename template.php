...
function tcesp_clean_navigation($links) {

    $result = array();
    foreach($links as $id => $item) {

        $new_item = array('title' => $item['link']['title'], 'link_path' => $item['link']['link_path'], 
'href' => $item['link']['href']);

        if ($item['below']) {
            $new_item['below'] = tcesp_clean_navigation($item['below']);
        }
        $result[] = $new_item;
    }
    return $result;
